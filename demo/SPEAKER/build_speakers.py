...
def build_speakers(
    ...
    line_tags, stats = assign_speakers(chunks, spans, min_score=min_score)
    ...
    print(f"[speaker] {stats.n_tagged}/{stats.n_lines} dong co cluster "
          ...
          f"{sum(1 for n in names if n)}/{len(names)} dong co ten", flush=True)
    return names, line_tags, mapping, stats
...
def run(
    ...
    names, line_tags, mapping, stats = build_speakers(
        chunks, spans, character_names, generate_fn, captions_text,
        min_score=min_score, count_tokens=count_tokens,
    )
...
def main() -> None:
    ...
    run(args.tagged_json, args.en_srt, args.registry_json, args.output_json,
        ...
        args.max_new_tokens, args.min_score)
