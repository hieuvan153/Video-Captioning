...
def score_dir(pred_dir: str, subs: dict[str, str], only: set[str] | None = None) -> dict[str, float]:
    ...
    for pf in sorted(glob.glob(os.path.join(pred_dir, "*.srt"))):
        ...
        if only and name.replace(".(Tiếng Việt).srt", "") not in only:
            continue
...
def main() -> None:
    ...
    only = None
    if "--movies" in sys.argv:   # ten tap (S05E015_...) sau --movies; cac dir dung truoc
        i = sys.argv.index("--movies"); only = set(unicodedata.normalize("NFC", x) for x in sys.argv[i + 1:]); sys.argv = sys.argv[:i]
    subs = {unicodedata.normalize("NFC", os.path.basename(s)): s
            for s in glob.glob(os.path.join(SUB_DIR, "*.srt"))}
    for d in sys.argv[1:]:
        r = score_dir(d, subs, only)
